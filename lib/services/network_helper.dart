...
// };
  ...
  static Future<dynamic> getMedicalHistory() async {
    ...
    var response = await Helper.get(
        url: GET_MEDICAL_RECORDS + '${id.toString()}',
        bearerToken: token.toString());
    print(response.statusCode);
    ...
    if (response.statusCode == 200) {
      return MedicalRecords(
        diseases: (result['diseases'] as List).map((disease) =>
            Disease.fromJson(disease)).toList(),
        medicines: (result['medicines'] as List).map((medicine) =>
            MedicineModel.fromJson(medicine)).toList(),
        surgeries: (result['operations'] as List).map((surgery) =>
            SurgeryModel.fromJson(surgery)).toList(),
        tests: (result['tests'] as List)
            .map((test) => TestModel.fromJson(test))
            .toList(),
      );
    }
    else if (response.statusCode == 404 &&
        result == 'This user doesn\'t have any medical history yet.') {
      return 404;
    }
    else
      return 0;
...
class Helper {
  ...
  static Future<http.Response> get(
      {@required String url, @required String bearerToken}) {
